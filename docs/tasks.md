...
- [ ] **Phase 4: ユーザー体験の向上**
  ...
  - [ ] **AI: 推論モード切り替え (Speed vs Quality)**

    - [ ] **Backend:**

      - [ ] `SoloConfig` スキーマに `model_mode` (Flash/Pro) を追加

      - [ ] `gemini.py` でモデルを動的に切り替えるロジックの実装

    - [ ] **Frontend:**

      - [ ] `GenerationRequest` 型定義の更新

      - [ ] 設定画面 (Settings) へのモード選択スイッチ追加
