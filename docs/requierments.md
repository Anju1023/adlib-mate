...
- **音声合成:**
  - 標準実装: **`Tone.js`** (Web Audio API)
    - 役割: クライアントサイドで即座にシンセサイザー音源を生成し、低遅延で再生。
  - バックログ: `Lyria 2 (lyria-002)` - API 権限が確保され次第、高品質なオーディオ生成として統合。
...
- **楽譜ビューア:**
  ...
  - 特徴: **再生カーソルとの同期機能**を実装し、視覚的に演奏箇所を把握可能。
- **練習モード:**
  - 機能: PWA 対応により、ホーム画面に追加してアプリとして利用可能。
